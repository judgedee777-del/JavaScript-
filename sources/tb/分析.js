...
// 统一的params构建函数
function build_params(page, keyword) {
    return {
        device: "HMA-AL00",
        isBeta: "false",
        grayHair: "false",
        ...
        startPrice: null,
        endPrice: null,
        p4pIds: "941599149966,756975608493,649521595240,649563635825,976496560678,740921683686,668464962077,861378231680,805591002301,965959156136,634000985622,771500914086,906466312237,919775521542,814890248861,834153980696,977936503466,746381523878,970540430137,954790084027,956160573939,954108837235,964927397161,923796578802,838141448784,805423105937,960264273499,971072576695,959860062899,45886329270,964139156803,959154338747,961381600465,977462899707,974295433285,902209641653,951241634446,979358708959,966941221020,945853764061,964338822221,969646322155,904900179879,947917690952,745031571457,842543353038,964200611417,712462656053",
        categoryp: "",
        myCNA: "EPLOID1viUsBASQIgjwmgsdT",
        clk1: "82deedb20813a0e349ef7e87e2c04056",
        refpid: "mm_2898300158_3078300397_115665800437"
    };
}

// 获取token时用的params
function get_token_params(keyword, page = 1) {
    var params = build_params(page, keyword);
...
// 搜索时用的签名
function get_sign(page, keyword, token) {
    var params = build_params(page, keyword);
    var data = JSON.stringify({
        appId: "43356",
        params: JSON.stringify(params)
    });
    var l = c(token + "&" + u + "&" + s + "&" + data);
...
// console.log(get_sign(4, "相机", "24124d7cb66d83f1e91a037fc43c60a2"))
